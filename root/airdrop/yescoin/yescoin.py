...
def parse_arguments():
    ...
    if args.task is None:
        task_input = 'n' #input("Apakah Anda ingin cek dan claim task? (y/n, default n): ").strip().lower()
        args.task = task_input if task_input in ['y', 'n'] else 'n'
    
    if args.multi is None:
        multi_input = 'n' #input("Apakah Anda ingin upgrade multi value? (y/n, default n): ").strip().lower()
        args.multi = multi_input if multi_input in ['y', 'n'] else 'n'
    
    if args.fill is None:
        fill_input = 'n' #input("Apakah Anda ingin upgrade fill rate? (y/n, default n): ").strip().lower()
        args.fill = fill_input if fill_input in ['y', 'n'] else 'n'
    
    if (args.multi == 'y' or args.fill == 'y') and args.max_level is None:
        max_level_input = 'n' #input("Masukkan level maksimum untuk upgrade (default: 5): ").strip()
