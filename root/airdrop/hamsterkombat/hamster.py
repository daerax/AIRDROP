...
def klaim_MiniGame(token, kode):
    url = 'https://api.hamsterkombatgame.io/clicker/apply-promo'
    headers = get_headers(token)
    headers['accept'] = 'application/json'
    headers['content-type'] = 'application/json'
      
    response = requests.post(url, headers=headers, json={"promoCode": kode})
...
def main():
    ...
    totalAkun = len(init_data)
    print(f"TOTAL AKUN: {totalAkun}")
            ...
            if response.status_code == 200:
                ...
                if auto_minigame == 'y':
                    ...
                    if token not in claimed_minigame:
                        ...
                        namaGame: list = ['bike', 'clone', 'cube', 'train', 'merge', 'twerk', 'mow']
                        for i in namaGame:
                            with open(fr'keygen/voucher_{i}.txt', 'r') as file:
                                lines = file.readlines()  # Baca semua baris dalam file
                                kode = lines[0].strip()  # Ambil baris pertama dan hapus karakter newline
                            response = klaim_MiniGame(token, kode)
                            try:
                                if response.status_code == 200:
                                    print(Fore.GREEN + Style.BRIGHT + f"\r[ Mini Game	] : Berhasil Klaim MiniGame [{i.upper()}]", flush=True)
                                    with open(fr'keygen/voucher_{i}.txt', 'w') as file:
                                    ...
                                    elif error_info.get('error_code','') == 'InvalidPromoCode':
                                        print(Fore.BLUE + Style.BRIGHT + f"[ Mini Game	] : {error_info.get('error_message', 'mbuh')} | menghapus CODE!", flush=True)
                                        with open(fr'keygen/voucher_{i}.txt', 'w') as file:
                                            file.writelines(lines[1:])
                                    else:
                                        print(Fore.RED + Style.BRIGHT + f"\r[ Mini Game	] : Gagal main {i.upper()}: {error_info.get('error_message', 'No error message')}", flush=True)
                            except json.JSONDecodeError:
