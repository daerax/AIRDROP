...
class Midas:
    def __init__(self) -> None:
        ...
        self.faker = Faker()
        self.proxies = {
            'http': 'http://viserion:tgryn@192.168.1.1:7890',
            'https': 'http://viserion:tgryn@192.168.1.1:7890',
        }
        self.headers = {
    ...
    def register(self, queries: str):
        ...
        for query in queries:
            ...
            try:
                response = self.session.post(url=url, headers=headers, proxies=self.proxies, data=data)
    ...
    def user_visited(self, token: str):
        ...
        try:
            response = self.session.patch(url=url, headers=headers, proxies=self.proxies)
            response.raise_for_status()
            return True
    ...
    def user(self, token: str):
        ...
        try:
            response = self.session.get(url=url, headers=headers, proxies=self.proxies)
            response.raise_for_status()
    ...
    def post_streak(self, token: str, first_name: str, points: int, tickets: int):
        ...
        try:
            response = self.session.post(url=url, headers=headers, proxies=self.proxies)
            response.raise_for_status()
            streak = response.json()
            return self.print_timestamp(
                f"{Fore.CYAN + Style.BRIGHT}[ {first_name} ]{Style.RESET_ALL}"
    ...
    def main(self, queries: str):
        while True:
            try:
                ...
                for i in range(2):      #Copy dari sini
                    i += 1
                    print(f"{i}x coba") #Sampai sini
                    total_points = 0
