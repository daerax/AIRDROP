...
updateURL = 'https://raw.githubusercontent.com/daerax/update/refs/heads/main/update.json'
baru = requests.get(updateURL).json()

DOMAIN_API = {
    "SESSION": baru['nodepay']['SESSION'],
    "PING": baru['nodepay']['PING']
}
...
async def main():
    all_proxies = load_proxies('1-10.txt')  
    # Take token input directly from the user
    token = 'eyJhbGciOiJIUzUxMiJ9.eyJzdWIiOiIxMzA2Mjk5Mjg2NjczMTYyMjQwIiwiaWF0IjoxNzMxNTE4NTg1LCJleHAiOjE3MzI3MjgxODV9.zYmVz9c_bi22H1fx2K2AF4N0aT4aBdTBYxX6e8qXEGsTtZp8DNGKe7B1AIaHiaIzWVZ95oTsp-hxOhEBHRAVIA'#input("Nodepay token: ").strip()
    if not token:
