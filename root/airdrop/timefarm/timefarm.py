...
def parse_arguments():
    ...
    if args.task is None:
        task_input = 'n' #input("Apakah Anda ingin auto claim task? (y/n, default n): ").strip().lower()
        args.task = task_input if task_input in ['y', 'n'] else 'n'
    
    if args.upgrade is None:
        upgrade_input = 'y' #input("Apakah Anda ingin auto upgrade clock? (y/n, default n): ").strip().lower()
